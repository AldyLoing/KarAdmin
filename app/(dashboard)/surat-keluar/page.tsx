...
export default function SuratKeluarPage() {
  ...
  const [filteredLetters, setFilteredLetters] = useState<OutgoingLetter[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingLetter, setEditingLetter] = useState<OutgoingLetter | null>(null);
    ...
    if (searchTerm) {
      const filtered = letters.filter(
        (letter) =>
          ...
          letter.tujuan.toLowerCase().includes(searchTerm.toLowerCase())
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (file) {
        const { data, error } = await uploadFile('documents', 'surat-keluar', file);
        if (error) throw error;
      ...
      if (editingLetter) {
        ...
        toast({
          title: 'Berhasil',
          description: 'Surat keluar berhasil ditambahkan.',
        });
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Kelola surat keluar dan dokumen terkait</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{editingLetter ? 'Edit' : 'Tambah'} Surat Keluar</DialogTitle>
              <DialogDescription>
                Lengkapi form di bawah untuk {editingLetter ? 'mengubah' : 'menambah'} surat keluar.
              </DialogDescription>
            </DialogHeader>
              ...
              {filteredLetters.length === 0 ? (
                ...
                filteredLetters.map((letter) => (
                  <TableRow key={letter.id}>
                    ...
                    <TableCell>{letter.tujuan}</TableCell>
